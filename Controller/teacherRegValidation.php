...
if(empty($userNameErr) && empty($passErr) && empty($emailErr) && empty($confirmPassErr)){
        ...
        $s = "select * from teacher where username = '$userName'";
        ...
        if($num == 1){
            echo "<script>
                    ...
                    window.location.href = '../View/teacherRegistration.php';
                </script>";
        }else{
            $reg =  "insert into teacher(username, email, password ) values ('$userName', '$email', '$pass')";
    ...
    include('../View/teacherRegistration.php');
}
?>
</body>
</html>